...
(function (global, factory) {
        ...
        return {
          pre: function pre($scope, $elm, $attrs, ctrl) {
            ...
            $scope.$watch("cell.row", function () {
              ...
              if(typeof ctrl.getValue() != 'string'){
                var html = '';
                for(var index=0;index < ctrl.getValue().length;index++){
                  html += '<div>'+ctrl.getValue()[index]+'</div>';
                }
                content[0].innerHTML = html;;
              }
              else if (ctrl.column.template) {
    ...
    _createClass(BodyController, [{
      ...
      value: function buildRowsByGroup() {
        ...
        for (var i = 0, len = this.rows.length; i < len; i++) {
          ...
          if (relVal) {
            ...
            row.isEven = this.rowsByGroup[relVal].length % 2 == 0;
          }
      ...
      value: function rowClasses(row) {
        var styles = {
          ...
          "dt-row-even": row && row.isEven,
          "dt-row-odd": row && !row.isEven
        };
  ...
  var DataTableController = (function () {
    ...
    _createClass(DataTableController, [{
      ...
      key: "onDeselected",
      value: function onDeselected(rows) {
        this.onDeselect({
          rows: rows
        });
      }
